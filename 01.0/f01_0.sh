...
/usr/local/bin/qemu-system-x86_64  -monitor stdio \
  ...
  -m 4096 \
  -smp 4 \
  -drive file=./fbsd01_0.iso,index=2,media=cdrom \
  -boot order=cda,menu=on \
  -blockdev driver=file,filename=./fbsd01_0.qcow2,node-name=myfile \
  ...
  -device virtio-net-pci,netdev=nd0,mac=56:00:00:01:00:00 \
  -name \"fbsd01.0\"
...
#    ln -s <filepath>/FreeBSD-01.0-RELEASE-<arch>-<bootfile>.iso  fbsd01_0.iso
# 2. Create a 2G hard disk image with qemu-img(1).
#    qemu-image create -f qcow2 -o preallocation=full fbsd01_0.qcow2 2G

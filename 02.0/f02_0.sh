...
/usr/local/bin/qemu-system-x86_64  -monitor stdio \
  ...
  -m 4096 \
  -smp 4 \
  -drive file=./fbsd02_0.iso,index=2,media=cdrom \
  -boot order=cda,menu=on \
  -blockdev driver=file,filename=./fbsd02_0.qcow2,node-name=myfile \
  ...
  -device virtio-net-pci,netdev=nd0,mac=56:00:00:02:00:00 \
  -name \"fbsd02.0\"
...
#    ln -s <filepath>/FreeBSD-02.0-RELEASE-<arch>-<bootfile>.iso  fbsd02_0.iso
# 2. Create a 2G hard disk image with qemu-img(1).
#    qemu-image create -f qcow2 -o preallocation=full fbsd02_0.qcow2 2G
